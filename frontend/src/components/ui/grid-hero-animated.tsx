  ...
  useEffect(() => {
    ...
    const drawLights = () => {
      ...
      lights.forEach((light) => {
        ...
        // Create glowing effect — amber/orange to match Findo branding
        const gradient = ctx.createRadialGradient(adjustedX, adjustedY, 0, adjustedX, adjustedY, 15)
        if (isDarkMode()) {
          gradient.addColorStop(0, `rgba(245, 158, 11, ${light.brightness})`)
          gradient.addColorStop(0.5, `rgba(217, 119, 6, ${light.brightness * 0.5})`)
          gradient.addColorStop(1, "rgba(180, 83, 9, 0)")
        } else {
          gradient.addColorStop(0, `rgba(245, 158, 11, ${light.brightness})`)
          gradient.addColorStop(0.5, `rgba(234, 88, 12, ${light.brightness * 0.5})`)
        ...
        ctx.fillStyle = isDarkMode()
          ...
          : `rgba(245, 158, 11, ${light.brightness})`
        ctx.beginPath()
